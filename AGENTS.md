...
- Treat `VERSION` as a public release identity, not a per-feature or per-commit development counter. Keep the last published version during ordinary development and change it only in the terminal release-preparation commit. Derive a new version from the latest stable public tag: exactly the next patch, the next minor with patch zero, or the next major with minor and patch zero. Release candidates start at `rc.1` and increment without gaps.
